...
function getDOB()
{
  ...
  apiBody.append("tag",'search');
  runAjax(GET_PARTICIPATE_DATA,apiBody,'par_dob');
}
...
function getdrama(catID,Container,extra)
{
...
function DramaTabFilm(parentCatid,Container,functionname)
{
    //alert(parentCatid+"--"+Container+"--"+functionname);
        console.log("cat id",parentCatid);
    ...
    if(functionname=='getDataFilmDrama')
    {
         _("drama-result").innerHTML='';
         //getDataFilmMusic(parentCatid,Container);
    }
}
...
function getMoviesCateorySubData(pcatID,Container)
{
     //var Container = "movies-result";
     //alert(Container);
     var apiBody = new FormData();
     apiBody.append("partnerid",PARTNER_ID);
     apiBody.append("categoryid",pcatID);
     apiBody.append("userid",Userid);
     apiBody.append("countrycode",countrycode);
     runAjax(GET_INFO, apiBody, Container);
}
...
/*function getMoviesubcategory(mainCatid){
