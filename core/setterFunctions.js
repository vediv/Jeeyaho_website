...
function buildMenu(JSONobject, Container,pagename,loginstatus,userid)
{
    ...
    var output = '<li><a href="javascript:void(0)" class="w3-left w3-hide-large w3-hide-medium" onclick="w3_open()"> <i class="fa fa-bars w3-padding-right w3-padding-left"></i></a> </li><li style="  border-bottom:0px !important" class="logo-right"> <a href="'+baseUrl+'/home" class="logo" style="  border-bottom:0px !important" >';
    ...
    for ( var i = 0; i < items ; i++)
    {
       ...
       // alert(catID);
        ...
        var urlCatName = JSONobject.Menu[i].catName.replace(/\s+/g, '-').toLowerCase();
        //console.log(catname1+"---"+catID+"---"+viewType);
        var url="#";
        ...
        {
            //url=baseUrl+"/category/"+urlCatName+"/"+catID;
            url=baseUrl+"/"+urlCatName+"/"+catID;
        }
        else if(viewType==6 || viewType=="6")
        {
            url=baseUrl+"/language/"+urlCatName+"/"+catID;
        }
         else if(viewType==5 || viewType=="5")
        {
             url=baseUrl+"/"+urlCatName+"/"+catID;
             //sessionStorage.setItem("music_id",catID);//console.log("pk ",CurrentPagename ,catID );
        }

        /*else if((viewType==2 && catname=='DEVOTIONAL') || (viewType=="2" && catname=='DEVOTIONAL'))
        {
        ...
        var act = (pagename==catID || (pagename=="jeeyaho" && catSm=="home")) ? "w3-active":"";
        ...
        output += '<div class="nav-padding"><li class="w3-left w3-hide-small nav-menu"> <a href='+url+' class='+act+'><img  src="'+menu_icon+'" class="height15"> '+catname+'</a> </li></div>';
        mobileOutput+='<a href='+url+' class='+act+'>'+catname+'</a>';
        }

        output +='<div style="float:left; margin:0 -40px 0 50px"><li class="w3-hide-small w3-left nav-menu" style="line-height: 18px !important; "><a class="w3-search "  title="Search" style="  border-bottom:0px !important" ><form action="' +baseUrl+ '/search" method="get"><input type="search"  required style="border-radius: 0px !important; margin:0px  !important; border-bottom: 1px solid #EEEEEE !important;;" name="searchtext" class="nav-search " value="' +search_request+ '"  /><button type="submit" class="nav-button"><i class="fa fa-search"></i></button></form></a></li></div>';
        output +=loginArea();
...
function buildCategoryWithDatafilm(JSONobject){
        control='<div class=" w3-row-padding" style="margin-top:20px">';
...
control+='<a onCLick="viewMedia(\''+entryid+'\',\''+plan+'\')"><img class="w3-image lazy"  src="'+thumburl+'" src="'+baseUrl+'/images/banners/placeholder@.png"></a>';
        ...
        control+='</div>';
         ...
         control +='</div>';
         //control +='<div class="w3-col s12 m12 l12 w3-center" id="button_load">';
      ...
      function buildCategoryWithDataWithFirstImage(JSONobject){

             control='<div class=" w3-row-padding" style="margin-top:1%">';
              var Category_data_len= JSONobject.Category_data.length;
              var entryid = JSONobject.Category_data.entryid;
              for(var i=0; i < Category_data_len; i++)
               {
...
function buildKidsCategory(JSONobject, Container){
      	...
      	var control='<div class="w3-row-padding">';
      	var Category_data_len= JSONobject.Category_data.length;
      	var entryid = JSONobject.Category_data.entryid;
        for(var i=0; i < Category_data_len; i++)
         {
...
function buildMoviesHtml(JSONobject, Container)
   {
       if(Container=='movies-result'){
	     var control='<div class="  w3-text-white " style=" ">';
	     //control+='<div class="carousel-inner">';
	     var catinfo_len= JSONobject.category_info.length;
	     ...
	     for(var i=0; i < catinfo_len; i++)
	     {
		     ...
		     var Parent_id = JSONobject.category_info[i].Parent_id;
		     var imgurl =JSONobject.category_info[i].t_original_url;
         var urlParent_name = Parent_name.replace(/\s+/g, '-').toLowerCase();
         //var urlLink="../movies11/"+Parent_name+"/"+Parent_id;
		     control+='<div class="w3-third  w3-center w3-row-padding margin-top20" style="margin-top: 2%; position:relative;margin-bottom: 1%;">';
		     control+='<a href="javascript:void(0)" class="" onclick=callViewAllPage(\''+Parent_id+'\',\''+urlParent_name+'\',"movies_all_url")><img class="img-res zoom"     data-src="'+baseUrl+'/images/placeholder1.png" src="'+imgurl+'" style="box-shadow: 3px 3px 20px rgba(0, 0, 0, .5);"> ';
...
function buildMoviesCategoryHtml(JSONobject, Container)
 {
        ...
        control+='<div class="w3-card-2">';
        control+='<a style="cursor:pointer;"><img class="lazy img-res-mov" data-src="'+thumburl+'"  onCLick="viewMedia(\''+entryid+'\',\''+plan+'\')" style="" ></a>';
        control+='<span class="duration">'+msToTime(duration)+'</span>';
...
function buildDramaHtml(JSONobject, Container)
  {
    var items= JSONobject.category_subdata.length;
     if(items<=0){_(Container).innerHTML ="<div><span class='w3-prussian_blue' style='margin-top:10%;font-size:19px;'> &nbsp;No videos available222.. &nbsp;<span></div>";return false;}
    var row = "";
    for (var i = 0; i < items ; i++)
    {
        var title_tag_name = JSONobject.category_subdata[i].title_tag_name.toUpperCase();
...
function buildMusicHtml(JSONobject, Container)
  {
    //alert(Container);
    var items= JSONobject.category_subdata.length;
    if(items<=0){_(Container).innerHTML ="<div><span class='w3-prussian_blue' style='margin-top:10%;font-size:19px;'> &nbsp;No videos available.. &nbsp;<span></div>";return false;}
    var row = "";
    for (var i = 0; i < items ; i++)
    {
        var title_tag_name = JSONobject.category_subdata[i].title_tag_name.toUpperCase();
        var Parent_id = JSONobject.category_subdata[i].categoryid;
        var search_tag_array_length = JSONobject.category_subdata[i].search_tag.length;
        if(search_tag_array_length>0)
        {

            row+='<p class="w3-content w3-container heading">'+title_tag_name;
            row+='<a href="javascript:void(0)" class="" style="float:right" onclick=callViewAllPage(\''+Parent_id+'\',\''+title_tag_name+'\',"music_view_all_url")>View all</a></p>'

            // row+='<div class="w3-col s12 m12 l12">';
            //   row+='<div class="w3-col s6 m6 l6">';
            // row+='<p class="w3-content w3-container heading">'+title_tag_name+'</p>';
            //   row+='</div>';
            //     row+='<div class="w3-col s6 m6 l6">';
            // row+='<a href="javascript:void(0)" style="float:right" onclick=callViewAllPage(\''+Parent_id+'\',\''+title_tag_name+'\',"music_view_all_url")>View all</a>'
            // row+='</div>';
            // row+='</div>';
            var slide='<div class="w3-row-padding bxslider" >';
            for (var j = 0; j < search_tag_array_length ; j++)
...
function callViewAllPage(pcatid,pcatname,viewtag)
{
      var urlCatName = pcatname.replace(/\s+/g, '-').toLowerCase();
      switch(viewtag)
      {
        case "movies_all_url":
        document.location = baseUrl + "/movies/"+urlCatName+"/"+pcatid;
        break;
        case "music_view_all_url":
        document.location = baseUrl + "/music/"+urlCatName+"/"+pcatid;
        break;
