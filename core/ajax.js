    ...
    switch(Container)
    {
        ...
        case "home-setting":
                if(_(Container))
                         ...
                         _(Container).innerHTML += '<div id="preloader1"><div id="status1">&nbsp;</div></div>';
        ...
        break;
                case "music-result":
        if(_(Container))
            _(Container).innerHTML = '<div id="preloader1" style="margin-top:7%"><div id="status1">&nbsp;</div></div>';
        break;
                case "liveTv":
        if(_(Container))
            _(Container).innerHTML = '<center><div class="slice borderRed" > <div data-loader="spinner"></div></div></center>';
     ...
     /*
       ...
       // var user='PlanetCastM'; var pass = 'p1@netc@$t';
        ...
        if(Container!="countrylist" && Container!="get_token")
        {
          httpObject.setRequestHeader("token",token);
        }
        //httpObject.setRequestHeader("HTTP_TOKEN",);
...
{//console.log("DEV_PRINT:login__"+JSONresponse+"__"+Container);
    ...
    // console.log('participate'+JSONresponse);
     if(!_(Container))
...
window.location="sessionDestroy.php?tokFlag="+TokenStatus;
    ...
    switch(Container)
    {
             ...
             case "par_dob":
                 ...
                 {
                   _('par_dob').value=JSONobject.dob;
                 }
                 else{
                   $('#pbutton').prop('disabled', true);
                   _('presponse').innerHTML=JSONobject.msg;
                   break;
                 }
               break;
             case "presponse":
                  $('#pbutton').prop('disabled', true);
                  _(Container).innerHTML=JSONobject.status;
                  break;
             case "Guest_user":
        ...
        case "navMain":
         ...
         break;
              ...
              case "home-setting":
              setTimeout(()=>buildHomeSetting(JSONobject, Container),2000); // this function called in js/functions.js
              break;
        ...
        break;
          ...
          break;
           ...
           break;
            ...
            case "music-result":
                setTimeout(()=>buildMusicHtml(JSONobject, Container),500);
            break;

             case "buildDramaMenu":
                buildDramaTabItems(JSONobject, Container);
           break;
            case "catgory_with_data":
                buildCategoryWithData(JSONobject, Container);
            break;
            case "drama-result":
                buildDramaHtml(JSONobject, Container);
           break;
             case "buildDevotionalMenu":
                buildDevotionalTabItems(JSONobject, Container);
           break;
             case "devotional-result":
                buildDevotionalHtml(JSONobject, Container);
            break;
            case "buildKidsMenu":
                buildKidsTabItems(JSONobject, Container);
           break;
             case "dKids-result":
                buildKidsHtml(JSONobject, Container);
            break;
