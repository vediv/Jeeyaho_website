...
<?php
$uri = $_SERVER['REQUEST_URI']; $uri_array = explode( "/", $uri);
$catID=isset($_GET['catID'])?$_GET['catID']:'';
$pcatname=isset($_GET['catname'])?$_GET['catname']:'';
$subcatid=end($uri_array);
?>
...
alert(subcatid);
var pcatname="<?php echo $pcatname; ?>";
function buildMusicTabItems(JSONobject)
{
        ...
        if(len>0)
        {
          ...
          var subCount=datan.sub_count;
        	if(Parent_id==subcatid){ getDataWithCategory(Parent_id,"music-result"); $("#load_more_button").hide();
        	  var containerNew="music-result";
            alert("yes");
             }
            if(subCount>0 && subcatid=='')
            {
              alert("yes123");
            ...
            if(Parent_id!=pcatid && pcatid!='')
            {
              ...
              $("#load_more_button").hide();
              getDataWithCategory(pcatid,"music-result"); $("#load_more_button").hide();
            	var containerNew="music-result";
...
function callTab(cid,cname)
{
    ...
    document.location = baseUrl+"/detail/"+pcatname+"/"+cid+"/"+ncname+"/"+cid;
    //return false;
}
